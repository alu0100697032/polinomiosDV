...
polinomio & polinomio :: operator * (const polinomio & y){
    polinomio aux((getGrado() * y.getGrado()), (getTerminos() * y.getTerminos()));
...
polinomio polinomio::obtener_pi(){
    polinomio aux(getGrado(), getTerminos()/2);
    for (int i = 0; i < getTerminos()/2; i++){
        aux.poli[i] = poli[i];
    }
    return aux;
}
...
polinomio polinomio::obtener_pd(){
    polinomio aux(getGrado(), getTerminos()/2);
    for (int i = 0; i < getTerminos()/2; i++){
        aux.poli[i] = poli[i + getTerminos()/2];
    }
    return aux;
}
/*
...
polinomio polinomio::multiplica (const polinomio & y){
    ...
    for (int i = 0; i < getTerminos(); i++){
        for (int j = 0; j < y.getTerminos(); j++){
            aux.poli[poli[i].getExp()+y.poli[j].getExp()] = aux.poli[poli[i].getExp()+y.poli[j].getExp()] + (poli[i] * y.poli[j]);
        }
    }
    return aux;
}
